...
*/
  ...
  private int GetNrOfFormalParams(FunDecl f) {
    ...
    Decl D = f.paramsAST;
    assert(
      (D instanceof EmptyFormalParamDecl) ||
  ...
  public void visit(BinaryExpr x) {
    ...
    if (HasIntOrFloatArgs(x.oAST)) {
      if (
        x.lAST.type.Tequal(StdEnvironment.intType) &&
        x.rAST.type.Tequal(StdEnvironment.intType)
      ) {
      ...
      if (
        x.lAST.type.Tequal(StdEnvironment.boolType) &&
    ...
    x.type = StdEnvironment.errorType;
    if (!(
      (x.lAST.type instanceof ErrorType) ||
